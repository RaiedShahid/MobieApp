...
export default class ImageOptionForVehicle extends React.Component {
	constructor() {
		super();
		this.state = {
			personInfo: {
	...
	handleSubmit = async (e) => {
		...
		// console.log(result);
		const Info = this.props.navigation.getParam("personInfo");
		...
		try {
			this.state.personInfo.mobilename &&
				(await axios
					.put(`http://192.168.18.144:4000/home/mobile/${Info._id}`, formData)
					.then((res) => {
						// window.location = "/";
			...
			this.state.personInfo.carname &&
				(await axios
					.put(`http://192.168.18.144:4000/home/vehicle/${Info._id}`, formData)
					.then((res) => {
						// window.location = "/";
						console.log(res.data);
						// this.props.navigation.navigate("Home");
					...
					.catch((err) =>
						console.log("Some problem in Image Option Vehicle.js ", err)
					));
			this.state.personInfo.name &&
				(await axios
					.put(`http://192.168.18.144:4000/home/${Info._id}`, formData)
					.then((res) => {
						// window.location = "/";
						console.log(res.data);
						// this.props.navigation.navigate("Home");
